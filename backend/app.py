...
def parse_with_gemini(ocr_text: str, format_template: Optional[dict] = None) -> dict:
    prompt = f"""
    Extract invoice data from the following text into JSON format.
    Return ONLY valid JSON, no other text.
    
    Fields to extract:
    - vendor_name: The company name issuing the invoice
    ...
    if format_template:
        prompt += f"\n\nUse this as reference format: {json.dumps(format_template)}"
...
async def upload_invoice(file: UploadFile = File(...)):
    try:
        ...
        extracted = parse_with_gemini(ocr_text, template_config)
